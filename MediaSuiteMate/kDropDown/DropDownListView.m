...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        _kTableView.separatorColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:.2];
        ...
        if (isMultipleSelection) {
            ...
            [btnDone setFrame:CGRectMake(rect.origin.x+180,10, 80, 30)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.arryData containsObject:indexPath]){
        imgarrow.image=[UIImage imageNamed:@"checkbox_Selected"];
	} else
        imgarrow.image=[UIImage imageNamed:@"checkbox_default"];
